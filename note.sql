...
INSERT INTO `vnues`.`students` (`id`, `name`, `sex`, `age`, `tel`) VALUES (2124, 'vn', '女', 23, '15363383750')

# 去重

SELECT DISTINCT tel FROM students;

SELECT  tel FROM students;
...
SELECT CONCAT(tel,id) AS 电文 FROM students;
...
SELECT tel FROM students WHERE tel>=15363383750;
...
#  主句查询范围在列 字句范围为行 --> 为什么我查询的是tel列 但是字句可以扩展到id列  这就是原因
SELECT tel FROM students WHERE tel>=1536338375&&id>=2019;

SELECT tel FROM students WHERE tel>=1536338375&&id>=9919;
...
# 进阶三 排序查询 order by asc desc 升序还是降序(从高到底)
...
SELECT * FROM students WHERE id >1996 ORDER BY tel ASC;
...
SELECT '你好'  FROM employees;

SELECT '你好' AS 无中生有 FROM employees;
...
SELECT  STR_TO_DATE('4-3 1992','%c-%d %Y') AS date; # 1992-04-03 格式最后是这样

SELECT  STR_TO_DATE('1992-4-3','%Y-%c-%d') AS date; # 效果一样
...
SELECT MAX(salary) AS '最高工资',job_id FROM employees GROUP BY job_id;
...
SELECT job_id FROM employees GROUP BY job_id; # 19

# 查询每个位置的部门个数  分组查询 你就是可以将以条件分组得到的组进行使用  返回的都是组及的 要操作都是都是对组操作的
# 这个返回的值实际是一个组 这个组是封闭 你可以看成 不可以从里面取值 但是你可以使用分组函数进行操作
# 字句查询的是行

SELECT MAX(salary) AS '最高工资',job_id,employee_id FROM employees GROUP BY job_id;
...
SELECT COUNT(*) AS 部门人数,department_id FROM employees GROUP BY department_id ;

# 好像这样要求只能分组后查询
SELECT COUNT(*) AS 部门人数,department_id FROM employees GROUP BY department_id HAVING COUNT(*)>2;


# 统计行数 只要该行中有一列不是null 就统计上去
SELECT COUNT(*) FROM employees;

SELECT COUNT(1) FROM employees; # 意思是加多一列1这样的 你数据库有多少行就加多少行 但这不是写入的（真正加入的）

# 类似的

SELECT COUNT('lin xiao shan') FROM employees;
...
# 你可以这样理解 字句筛选出来的数据 ->SELECT 根据后面的字段A ->A去对应的B集合哪里拿对应的B字段 ---这个理解很重要
...
SELECT job_id FROM employees; # FROM employess employess是一个数据库表 字句比如where id>10是对这个数据库表作为筛选(整张表不是列) 然后job_id是决定你要从这个数据库拿什么字段
# 不存在主句拿到的是列 字句拿到的行的乱七八糟的操作
# SELECT manage_id FROM employees WHERER job_id>10; 以前存在的疑惑是主句是manage_id 而子句是job_id不是查询一一对应的 现在就比较好解释了 要知道行为如何和作用对象
...
SELECT name,boyName FROM beauty AS be,boys AS bo WHERE beauty.boyfriend_id=boys.id;

# 数据库表用别名 前面的主句查询列可以用be.name 因为就是从表取的 根据执行顺序我先走 SELECT name,boyName FROM再取SELECT name,boyName
...
FROM employees
GROUP BY job_id
ORDER BY MIN(salary) DESC; # 20个小组
...
SELECT MIN(salary),job_id,department_id,COUNT(*) # 21个小组
FROM employees
...
# https://blog.csdn.net/lin74love/article/details/80856067
...
SELECT department_id,city FROM locations INNER JOIN departments d on locations.location_id = d.location_id
 ...
 SELECT last_name,d.department_id,department_name FROM employees,departments AS d WHERE employees.department_id=d.department_id;


 # 查询90号部门员工的job_id和90号部门的location_id (分析为了显示查询的这两个字段job_id和location_id在不同的表里所以需要连表查询--以字段判断是不是多张表)
 # 这里为什么要关联 因为同个部门 筛选出job_id location_id 所以需要连表 这就是关联的原因
 # (employees表没有location_id)
 SELECT e.job_id,d.location_id,e.department_id FROM employees AS e,departments AS d WHERE d.department_id=e.department_id AND d.department_id=90;
...
SELECT * FROM jobs GROUP BY job_id;

SELECT job_id FROM jobs GROUP BY job_id;

SELECT MIN(salary),job_id,department_id,COUNT(*) #分组 组内的行数（对而且每行必定有这个XY字段且每行的 X，Y都相同，不是X和Y相等）
FROM employees
GROUP BY department_id,job_id
ORDER BY MIN(salary) DESC;
